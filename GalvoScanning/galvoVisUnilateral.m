...
visCtx_x1 = [1.5,1.5,2.5,2.5];
visCtx_y1 = [6,5,5,6];
visCtx_x2 = [-1.5,-1.5,-2.5,-2.5];
visCtx_y2 = [6,5,5,6];
offTarget_x = [1.5,1.5,2.5,2.5];%list of false stimulation sites, must be the same size as target_x/y
offTarget_y = [10,11,10,11];% these should also make an area with the same distances as the target sites
moveList_x1 = [];
moveList_y1 = [];
moveList_x2 = [];
moveList_y2 = [];
fakeList_x = [];
fakeList_y = [];
nspots = length(visCtx_x1);
stim_rate = 1/40;%stimulation rate of 40Hz
stimProb= 0.1; %probability at which stimulation occurs
for i =1:nspots    
    moveList_x1 = [moveList_x1,1, repmat(visCtx_x1(i)*xConv,1,s2.Rate*stim_rate/nspots)];%generates lists for the galvos to move such that each of 8 spots is stimulatedat 40 hz
    moveList_y1 = [moveList_y1,1, repmat(visCtx_y1(i)*yConv,1,s2.Rate*stim_rate/nspots)];
    ...
    moveList_x2 = [moveList_x2,1, repmat(visCtx_x2(i)*xConv,1,s2.Rate*stim_rate/nspots)];%generates lists for the galvos to move such that each of 8 spots is stimulatedat 40 hz
    moveList_y2 = [moveList_y2,1, repmat(visCtx_y2(i)*yConv,1,s2.Rate*stim_rate/nspots)];

end
moveList_x1 = repmat(moveList_x1,1,600);
moveList_y1 = repmat(moveList_y1,1,600);%pregenerated list of targets for stimulation on the mouse
fakeList_x = repmat(fakeList_x,1,600);
fakeList_y = repmat(fakeList_y,1,600);%pregenerated list that is off the mouse to use as false stimulation
moveList_x2 = repmat(moveList_x2,1,600);
moveList_y2 = repmat(moveList_y2,1,600);%pregenerated list of targets for stimulation on the mouse
%% Main execution loop    
XY_list = double.empty(0,length(offTarget_x)*2);

while true
if rand(1) <= stimProb/2 %choose where the stimulation will be based on probability
    outputx = moveList_x1';
    dest_x = visCtx_x1;
    outputy = moveList_y1';
    dest_y = visCtx_y1;
elseif rand(1) <= stimProb
    outputx = moveList_x2';
    dest_x = visCtx_x2;
    outputy = moveList_y2';
    dest_y = visCtx_y2;
...
try
    ...
    fprintf('Triggered!')
    XY_list = [XY_list; [dest_x,dest_y*0]];
